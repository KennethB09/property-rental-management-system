...
export default function ChatItem({
  ...
  return (
    <div className="flex h-20 w-full gap-4 items-center" onClick={onClickConversation}>
      ...
      <div className="flex flex-col w-full min-w-0">
        ...
        <div className="flex justify-between w-full overflow-hidden text-gray-500 min-w-0">
          <span className="truncate">{conversation.last_msg && sender + ": " + conversation.last_msg.content}</span>
          <span className="flex-shrink-0 ml-2">
            {" • "}{conversation.last_msg && format(new Date(conversation.last_msg.created_at), "hh:mm aa")}
          </span>
        </div>
      </div>
    </div>
